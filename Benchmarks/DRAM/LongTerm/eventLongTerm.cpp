...
std::chrono::milliseconds oneminute(60000);
std::chrono::milliseconds tenminutes(600000);
std::chrono::milliseconds onehour(3600000);
std::chrono::milliseconds tenhours(28800000);
...
int main(int argc, char *argv[]){
	...
	std::cout << "Buffer sizeB: " << size*sizeof(int)/(1024*1024) << "MB\n";
	...
	HIP_SAFE_CALL(hipDeviceSynchronize());
		...
		std::cout << "Apply DVFS status: " << status << "\n";
	#endif
	...
	if(status == 0) {
		...
		std::cout << "Registered time: " << n_time[0][0] << " ms \n";
		...
		HIP_SAFE_CALL(hipDeviceSynchronize());
		printf("Start Sleep 1 minute\n");
		std::this_thread::sleep_for(oneminute);
		printf("End Sleep\n");
		HIP_SAFE_CALL(hipDeviceSynchronize());
		...
		HIP_SAFE_CALL(hipDeviceSynchronize());
		printf("Start Sleep 10 minute\n");
		std::this_thread::sleep_for(tenminutes);
		printf("End Sleep\n");
		HIP_SAFE_CALL(hipDeviceSynchronize());
		...
		HIP_SAFE_CALL(hipDeviceSynchronize());
		printf("Start Sleep 1 hour\n");
		std::this_thread::sleep_for(onehour);
		printf("End Sleep\n");
		HIP_SAFE_CALL(hipDeviceSynchronize());
		...
		HIP_SAFE_CALL(hipDeviceSynchronize());
		printf("Start Sleep 8 hours\n");
		std::this_thread::sleep_for(tenhours);
		printf("End Sleep\n");
		HIP_SAFE_CALL(hipDeviceSynchronize());
