...
template <class T> __global__ void benchmark(T* cdin0, T* cdout0){
...
int main(int argc, char *argv[]){
	...
	if(pid == 0) {
		...
		if(status == 0) {
			...
			runbench(&n_time_verification[0][0],&value_verification[0][0], deviceIn, deviceOut);
			...
			// Transfer data from device to host
			HIP_SAFE_CALL(hipMemcpy(hostOutverification, deviceOut,  size*sizeof(int), hipMemcpyDeviceToHost));
