...
public:
        ...
        conv_desc_(pad_h, pad_w, hstride, wstride)
    {
        ...
        std::cout << std::endl;
        for (int i = 0; i < requestAlgoCountV; ++i)
        {
          if(perfResultsv[i].time != 0.0f && !isnan(perfResultsv[i].time))
            std::cout << std::setw(15) << get_fwd_algo_string_print(perfResultsv[i].fwd_algo) << " - " << std::setw(8) << std::setprecision(5) << perfResultsv[i].time << " - " << std::setw(8) << perfResultsv[i].memory << std::endl;
        }
        
        fwd_algo_ = perfResultsv[0].fwd_algo;
...
std::tuple<int, int, int, std::string, std::string, std::string> time_cnn(
    ...
    std::string bwd_params_algo_s = cnn.get_bwd_weights_algo_string();
    // Warm up backward
    ...
    std::string bwd_inputs_algo_s = cnn.get_bwd_data_algo_string();
